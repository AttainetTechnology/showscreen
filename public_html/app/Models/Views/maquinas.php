...
<div class="d-flex justify-content-between mb-3">
    <button onclick="abrirModalAgregar()" class="boton btnAdd">Añadir Máquina
...
<div id="gridMaquinas" class="ag-theme-alpine" style="height: 600px; width: 100%;"></div>

<div class="modal fade" id="editModal" tabindex="-1" aria-labelledby="editModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="editModalLabel">Añadir Máquina</h5>
            ...
            <div class="modal-body">
                <form id="editMaquinaForm">
                    ...
                    <input type="hidden" name="id_maquina" id="id_maquina">
                </form>
            </div>
            <div class="modal-footer">
                <button type="button" class="boton btnAdd" id="saveEditBtn">Guardar Máquina
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        const columnDefs = [
            {
                ...
                cellRenderer: params => {
                    ...
                    return `
                    <button onclick="editarMaquina('${links.editar}', '${params.data.nombre}', '${params.data.id_maquina}')" class="btn botonTabla btnEditarTabla">Editar
                    ...
                    <button onclick="eliminarMaquina('${links.eliminar}')" class="btn botonTabla btnEliminarTabla">Eliminar
            ...
            { headerName: "ID", field: "id_maquina", filter: "agTextColumnFilter", hide: true },
        ...
        const gridDiv = document.querySelector('#gridMaquinas');
        new agGrid.Grid(gridDiv, gridOptions);

        // Evento del botón para quitar filtros
        document.getElementById('clear-filters').addEventListener('click', function () {
            gridOptions.api.setFilterModel(null); // Resetea los filtros
            gridOptions.api.onFilterChanged();   // Refresca la tabla
        });

        function fetchMaquinas(gridApi) {
            fetch('<?= base_url("maquinasController/getMaquinas") ?>')
    ...
    function abrirModalAgregar() {
        $('#editModalLabel').text('Añadir Máquina');
        $('#nombre').val('');
        $('#id_maquina').val('');
        isEditing = false;
    ...
    function editarMaquina(url, nombre, idMaquina) {
        $('#editModalLabel').text('Editar Máquina');
        $('#nombre').val(nombre);
        $('#id_maquina').val(idMaquina);
        isEditing = true;
        $('#editModal').modal('show');
    }

    $('#saveEditBtn').click(function () {
        const url = isEditing ? '<?= base_url("maquinasController/actualizarMaquina") ?>' : '<?= base_url("maquinasController/agregarMaquina") ?>';
        $.post(url, $('#editMaquinaForm').serialize(), function (response) {
            if (response.success) {
                location.reload();
            } else {
