                    ...
                    <tr data-familia="<?= esc($producto['id_familia']) ?>">
                        ...
                        <td>
                            <?php if ($producto['id_producto'] == $id_producto_venta): ?>
                                <button type="button" class="boton btn btn-danger btn-select"
                                    data-id="<?= esc($producto['id_producto']) ?>" data-action="deselect">
                                    Desactivar <i class="bi bi-check-square ms-1"></i>
                                </button>
                            <?php else: ?>
                                <button type="button" class="boton btn btn-success btn-select"
                                    data-id="<?= esc($producto['id_producto']) ?>" data-action="select">
...
<script>
    $(document).ready(function() {
        ...
        // Evento para manejar la selección/deselección de producto
            ...
            $.post('<?= base_url('productos_necesidad/actualizarProductoVenta') ?>', {
                ...
                if (response.success) {
                    if (action === 'select') {
                        ...
                        // Actualiza el nombre del producto asociado
                        $('.asociado-producto').text('Producto asociado: ' + nombreProducto);
                    } else {
                        ...
                        // Limpia el nombre del producto asociado
                        $('.asociado-producto').text('No hay producto de venta seleccionado');
                    }
                } else {
