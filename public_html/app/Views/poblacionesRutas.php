...
<br>
<h2 class="tituloPoblaciones">Poblaciones y Rutas</h2>
<br>
<div class="d-flex justify-content-between mb-3 btnPoblacion">
    <button onclick="abrirModalAgregar()" class="boton btnAdd">Añadir Población
...
<div id="gridPoblaciones" class="ag-theme-alpine" style="height: 600px; width: 100%;"></div>

<!-- Modal para agregar/editar poblacion -->
<div class="modal fade" id="editModal" tabindex="-1" aria-labelledby="editModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="editModalLabel">Añadir Población</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Cerrar"></button>
            </div>
            <div class="modal-body">
                <form id="editPoblacionForm">
                    <div class="mb-3">
            ...
            <div class="modal-footer">
                <button type="button" class="boton btnGuardar" id="saveEditBtn">Guardar Población
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        const columnDefs = [
            {
                ...
                cellRenderer: params => {
                    ...
                    return `
                 <button onclick="editarPoblacion('${links.editar}', '${params.data.poblacion}', '${params.data.id_poblacion}')"
                class="btn botonTabla btnEditarTabla">Editar
                    ...
                    <button onclick="eliminarPoblacion('${links.eliminar}')" class="btn botonTabla btnEliminarTabla">Eliminar
            ...
            { headerName: "ID", field: "id_poblacion", filter: "agTextColumnFilter", hide: true },
        ...
        new agGrid.Grid(document.querySelector('#gridPoblaciones'), gridOptions);
    ...
    function fetchPoblaciones(gridApi) {
        fetch('<?= base_url("poblaciones_rutas/getPoblaciones") ?>')
            .then(response => response.json())
            .then(data => gridApi.applyTransaction({ add: data }))
    }

    function abrirModalAgregar() {
        $('#editModalLabel').text('Añadir Población');
        $('#poblacion').val('');
        $('#id_poblacion').val('');
        isEditing = false;
        $('#editModal').modal('show');
    }
    function editarPoblacion(url, poblacion, idPoblacion) {

        $('#editModalLabel').text('Editar Población');
        $('#poblacion').val(poblacion);
        $('#id_poblacion').val(idPoblacion); 
        isEditing = true;
        $('#editModal').modal('show');
    ...
    $(document).on('click', '#saveEditBtn', function () {
        const formData = $('#editPoblacionForm').serialize(); 


        const url = isEditing
            ? '<?= base_url("poblaciones_rutas/actualizarPoblacion") ?>'
            : '<?= base_url("poblaciones_rutas/agregarPoblacion") ?>';
    ...
    function eliminarPoblacion(url) {
        if (confirm('¿Estás seguro de que deseas eliminar esta población?')) {
            $.ajax({
                url: url,
                type: 'DELETE',
                success: function (response) {
