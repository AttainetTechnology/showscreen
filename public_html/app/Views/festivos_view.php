...
<div class="d-flex justify-content-between mb-3">
    <button class="boton btnAdd" onclick="abrirModalAgregarFestivo()">Añadir Festivo
...
<div id="festivosGrid" class="ag-theme-alpine" style="height: 400px; width: 100%;"></div>

<!-- Modal único para agregar o editar festivo -->
<div class="modal fade" id="addFestivoModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="addFestivoModalLabel">Añadir Festivo</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <form id="addFestivoForm">
                    <input type="hidden" id="id_festivo" name="id">
                    <div class="mb-3">
                        <label for="festivo" class="form-label">Nombre del Festivo</label>
                    ...
                    <div class="mb-3">
                        <label for="tipo_festivo" class="form-label">Tipo de Festivo</label>
                        <select class="form-control" id="tipo_festivo" name="tipo_festivo" required>
                            <option>Se repite anualmente</option>
                            <option>Varía anualmente</option>

                        </select>
                    </div>
                    <div class="d-flex justify-content-end">
                        <button type="button" class="boton btnGuardar" onclick="guardarFestivo()">Guardar Festivo
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        const columnDefs = [
            {
                ...
                field: "acciones",
                cellRenderer: params => `
                    <button onclick="editarFestivo(${params.data.id})"  class="btn botonTabla btnEditarTabla">Editar
                    ...
                    <button onclick="eliminarFestivo(${params.data.id}) " class="btn botonTabla btnEliminarTabla">Eliminar
            ...
            { headerName: "Nombre", field: "festivo", filter: 'agTextColumnFilter', minWidth: 150 },
            {
                headerName: "Fecha",
                field: "fecha",
        ...
        const gridOptions = {
            ...
            onGridReady: function (params) {
                fetchFestivosData(params.api); // Pasa correctamente params.api
            }

        };

        const gridDiv = document.querySelector('#festivosGrid');
    ...
    function fetchFestivosData(gridApi) {
        fetch('<?= base_url("festivos/getFestivos") ?>')
            .then(response => response.json())
            .then(data => {
                if (Array.isArray(data)) {
                    gridApi.applyTransaction({ add: data }); // Utiliza applyTransaction para añadir datos
                } else {
    ...
    function abrirModalAgregarFestivo() {
        $('#addFestivoModalLabel').text('Añadir Festivo');
        $('#id_festivo').val('');
        $('#addFestivoForm')[0].reset();
        $('#addFestivoModal').modal('show');
    }

    function guardarFestivo() {
        const formData = $('#addFestivoForm').serialize();
        const idFestivo = $('#id_festivo').val();
        const url = idFestivo ? `<?= base_url("festivos/actualizarFestivo") ?>/${idFestivo}` : '<?= base_url("festivos/agregarFestivo") ?>';

        $.post(url, formData)
            .done(response => {
                if (response.success) {
                    $('#addFestivoModal').modal('hide');
                    location.reload();
                } else {
    ...
    function editarFestivo(id) {
        $.get(`<?= base_url("festivos/getFestivo") ?>/${id}`, data => {
            $('#addFestivoModalLabel').text('Editar Festivo');
            $('#id_festivo').val(data.id);
            $('#festivo').val(data.festivo);
            $('#fecha').val(data.fecha);
            $('#tipo_festivo').val(data.tipo_festivo);
            $('#addFestivoModal').modal('show');
        });
    }

    function eliminarFestivo(id) {
        if (confirm('¿Deseas eliminar este festivo?')) {
            $.post(`<?= base_url("festivos/eliminarFestivo") ?>/${id}`)
                .done(response => {
                    if (response.success) {
                        location.reload();
                    } else {
