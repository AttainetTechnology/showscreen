            ...
            document.addEventListener('DOMContentLoaded', function () {
                ...
                function renderActions(params) {
                    ...
                    <button type='button' class="btn botonTabla btnEditarTabla btnEditarTablaLinea" data-id="${id}" data-bs-toggle="modal" data-bs-target="#editarLineaModal">
                        ...
                        <button type="button" class="btn botonTabla btnImprimirTabla" onclick="mostrarParte(${id})">
                        Parte
                        <svg xmlns="http://www.w3.org/2000/svg" width="26" height="26" viewBox="0 0 26 26" fill="none">
    ...
    var columnDefs = [
        {
            ...
            let botones = `
                ...
                <button type="button" class="btn botonTabla btnVerTabla" style="background: #e5eff7; color: #222;" onclick="mostrarPedidoTercerosPrint(${params.data.id_ped_terceros})">
    Imprimir
...
function mostrarPedidoTercerosPrint(id_ped_terceros) {
    $.ajax({
        url: '<?= base_url("Pedido_terceros_print/") ?>' + id_ped_terceros,
        type: 'GET',
        success: function (data) {
            $('#modalPedidoTercerosPrintContent').html(data);
            $('#pedidoTercerosPrintModal').modal('show');
        },
        error: function () {
            $('#modalPedidoTercerosPrintContent').html('<p class="text-danger">Error al cargar el pedido.</p>');
            $('#pedidoTercerosPrintModal').modal('show');
        }
    });
}
</script>
<!-- Modal para mostrar Pedido a Terceros Print -->
<div class="modal fade" id="pedidoTercerosPrintModal" tabindex="-1" aria-labelledby="pedidoTercerosPrintModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="pedidoTercerosPrintModalLabel">Pedido a Terceros</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body" id="modalPedidoTercerosPrintContent">
            </div>
        </div>
    </div>
</div>
<script>
document.addEventListener('DOMContentLoaded', function() {
    var modal = document.getElementById('pedidoTercerosPrintModal');
    if (modal) {
